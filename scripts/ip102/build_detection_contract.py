...
DEFAULT_OUTPUT = REPO_ROOT / "artifacts/data/ip102"
TEST_SOURCE = None
DATASET_REGRESSION_TEST_SOURCE = None
DATASET_SOURCE = None
MAPPING_SOURCE = None
DATASET_VERIFIER_SOURCE = REPO_ROOT / "scripts/ip102/verify_detection_contract.py"
...
def build(voc_root: Path, mapping_path: Path, taxonomy_path: Path, output: Path) -> dict[str, Any]:
    ...
    config = {
        "schema_version": SCHEMA_VERSION,
        "stage": "T0",
        "git_baseline": os.environ.get("T0_GIT_BASELINE", ""),
        ...
        "source_sha256": {"official_trainval": sha256_file(voc_root / "ImageSets/Main/trainval.txt"), "official_test": sha256_file(voc_root / "ImageSets/Main/test.txt"), "mapping": sha256_file(mapping_path), "taxonomy": sha256_file(taxonomy_path), "builder": sha256_file(Path(__file__).resolve())},
    }
...
"""
    ...
    checksum_paths = [output / name for name in OUTPUT_NAMES] + [Path(__file__).resolve(), mapping_path, taxonomy_path, voc_root / "ImageSets/Main/trainval.txt", voc_root / "ImageSets/Main/test.txt"]
