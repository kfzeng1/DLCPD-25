...
为了便于分类系统筛选和统计，本项目在不改变 203 个细粒度标签的前提下，采用“宿主作物 → 五大标签属性 → 具体标签”的层级。下表汇总第二层的五大属性。

| 层级 | 数量 | 示例 |
|---|---:|---|
| 一级：宿主作物 | 22 | 苹果、柑橘、玉米、水稻 |
| 二级：标签属性 | 5 | 农业有害生物、植物病害、健康、非生物或生理缺陷、混合或歧义 |
...
五大属性是项目级辅助元数据，不是论文作者发布的官方分类。模型的最终分类标签仍应是 203 个原始细粒度类别；宿主和五大属性可用于界面筛选、层级评估或级联分类。
