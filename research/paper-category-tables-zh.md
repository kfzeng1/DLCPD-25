...
为了便于分类系统筛选和统计，本项目在不改变 203 个细粒度标签的前提下，采用“宿主作物 → 四大标签属性 → 具体标签”的层级。下表汇总第二层的四大属性。

| 层级 | 数量 | 示例 |
|---|---:|---|
| 一级：宿主作物 | 22 | 苹果、柑橘、玉米、水稻 |
| 二级：标签属性 | 4 | 农业有害生物、植物病害、健康、非生物或生理缺陷 |
| 三级：具体标签 | 203 | 苹果 → 植物病害 → 苹果黑腐病 |

| 项目分组 | 类别数 | 定义 |
|---|---:|---|
| 农业有害生物 | 126 | 昆虫、螨类、腹足类等农业有害生物 |
| 植物病害 | 57 | 真菌、细菌、病毒、卵菌等病害或以病害命名的宽泛标签 |
| 健康 | 17 | 官方类别名明确标为健康的作物图片 |
| 非生物或生理缺陷 | 3 | 药害、红叶、花叶等异常 |
| **合计** | **203** | 逐类结果见 `metadata/class-taxonomy.json` |

四大属性是项目级辅助元数据，不是论文作者发布的官方分类。`garlic pest and diseases` 按项目约定归入植物病害，但原始标签名保持不变。模型的最终分类标签仍应是 203 个原始细粒度类别；宿主和四大属性可用于界面筛选、层级评估或级联分类。
