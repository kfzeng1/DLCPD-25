...
| `disease` | 真菌、细菌、病毒、卵菌等植物病害或以病害命名的宽泛标签 | 57 |
| `healthy` | 明确标为健康的作物图片 | 17 |
| `disorder` | 药害、红叶、花叶等非生物或生理异常 | 3 |

完整逐类结果见 `metadata/class-taxonomy.json` 和 `metadata/class-taxonomy.csv`。每条记录包含宿主组、宿主、四大类属性和具体标签；`class_id` 按 `metadata/official-class-names.txt` 的固定顺序从 0 编号。
...
- `garlic pest and diseases` 原名同时包含病害和虫害；为简化本项目层级，按项目约定归入 `disease`，但保留原始标签名。
...
脚本会检查 203 类是否都能唯一归入宿主和四大类，重写结构化分类清单，并在 `data/views/by-host/` 下生成不占用额外图片空间的三级软链接视图。
