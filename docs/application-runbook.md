...
本运行手册对应已经通过 F0 的 203 类历史分类应用。目标检测尚未接入当前 `7860` 服务；J5 完成后将在本文件中替换为单一联合模型配置、画框演示和联合排错步骤。
...
当前已发布版本只提供 203 类图像分类结果，不输出检测框。J5 目标版本将只加载一个联合模型，同时输出分类和 IP102 96 类害虫检测，但不会定位其余病害、健康和缺陷。低置信度或域外图片只显示不确定结果，不构成生产诊断。
