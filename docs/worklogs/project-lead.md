...
当前状态：F1 已通过；联合分类检测系统全部完成，默认 7860 服务可直接演示。

## 2026-08-13 完成态目录深度整理

- 清理：移除缓存、空验收目录、IP102 已解压压缩包、未引用的官方仓库浅克隆与空权重占位，以及已完成的 J3 一次性监督脚本和状态日志；删除项均进入系统回收站。
- 去重：只对 SHA-256 和字节逐项一致的冻结文件使用硬链接，保留全部原路径与 checksum 合同；覆盖 J1 两份相同 checkpoint、J3/J4 三份相同联合 checkpoint，以及 D4 两轮复现中 48 个与正式 D2/D3 相同的文件。
- 状态：修正计划、工程 README 和工程师日志首页的过期阶段文字；历史工作日志正文保留原始阶段记录，不改写历史。
- 边界：DLCPD-25 原图、IP102 检测图片/XML、固定 split、taxonomy、正式训练指标、最终模型包和 J5 浏览器证据均保留；203 个数据浏览软链接无断链。
- 复验：最终联合模型、J1、J3、T0、D4、D5 checksum 通过；J5 定向测试、Git 检查和 7860 在线检查通过。未重新读取或评估正式 test。
...
## 2026-08-13 J5 联合应用验收

- 已通过：唯一 J4 权重加载、统一 224 直缩、一次共享主干前向、203 类 Top-5、96 类害虫框、DLCPD-25 公共 ID、原图坐标反算、CPU/CUDA、真实 7860 `/analyze`、异常输入与能力边界文案。
- 工程证据：J5 定向 `7 passed`，相关兼容回归 `35 passed`，项目全量 `124 passed`；Ruff、`git diff --check`、HTTP 首页和 Gradio API 均通过。
- 浏览器：用户授权 Edge 9222 CDP 后完成桌面 `1650x785` 和移动 `390x844` 真实页面验收；上传 val 图片后低置信度提示、两张图、5 行 Top-5、2 行检测均正确。两种视口页面级无横向溢出，移动端标题、双图和字段响应式重排正常，无元素重叠。
- 结论：通过；允许进入 F1。浏览器截图保存在 `artifacts/audit/j5-browser/`。
...
- 保留：原始数据、`data-v1`/T0 数据合同、历史分类发布包、J1 分类初始化和唯一正式 J2 r8 链路验收产物；所有路径仍保持原样。
- 清理：所有撤销的 T1、J2 r1-r7、失败 J3 r1-r3、调试缓存，以及不再被代码引用的 A1/A2/A3 中间训练产物均移入系统回收站，可恢复；`artifacts/` 只保留正式 J1、J2 和发布包。
- 代码：新增 `training/joint.py` 承担两任务共享的优化器、梯度开关、检测拼接和 RNG 状态，J2/J3 不再互相调用私有实现；J3 改为主干/分类头 `1e-5`、检测头 `1e-4`，检测固定 FP32，并增加 85% 分类安全停止线、checkpoint RNG 恢复和 NumPy COCO JSON 序列化。
- 预检：首次只读检查发现 J2 r8 在小样本过拟合后完整分类 val Top-1 仅 `62.6973%`，确认它不能作为正式初始化。J3 已改为从 J1 构建共享主干/分类头并随机初始化检测分支；修正后只读预检 Top-1 为 `90.7882%`，通过 `88.783659%` 门槛。两次预检均未产生训练目录、训练 step 或 test 读取。
- 验证：J2/J3/检测模型/J1 定向回归 `26 passed`；最终项目全量回归 `111 passed`，仅 4 条既有 Gradio 6.0 弃用警告；新改训练代码 Ruff 与 `git diff --check` 均通过，冻结 D2-D5 校验和链保持有效。
- 风险：224 直缩会限制 IP102 小目标精度；J3 仍须由算法工程师在预检合格后启动，并仅用两个 val 选型。

## 2026-08-12 J2 验收与问题修复

- 结论：修复后通过；J2 已证明双数据集 1:1 交替、任务头梯度隔离、共享主干更新、一次共享前向和单一联合 checkpoint 可行，允许进入 J3；未读取任一 val/test，未执行 J3。
- 发现并修复：联合构造器原先会接受旧 A2/A3 权重，现强制要求 J1 阶段、RGB bicubic 直缩 `224x224` 和 test 隔离元数据；J2 原先只记录输入摘要，现对 J1 权重、DLCPD train/taxonomy、IP102 train/annotations/mapping 共 6 项 SHA-256 启动前强校验；checkpoint 原先未声明 DataLoader 中间位置，现升级 schema v2，只允许两个 loader 循环边界保存和恢复。
- 独立复验：修复后的正式产物为 `artifacts/training/detection/j2-alternating-smoke-95b24b9-r8/`；5 项 checksum 全部 OK；J2/T0 定向测试 `15 passed`；项目全量测试 `100 passed`，仅 4 条既有 Gradio 弃用警告；修改范围 Ruff 和 `git diff --check` 通过。
- 模型与训练：一个 ResNet-50 主干、203 类分类头、FPN/RPN/Faster R-CNN 96 类前景检测头；联合推理一次主干前向。分类 step 只更新共享主干和分类头，检测 step 只更新共享主干和检测头；24 个 CUDA step 的参数边界和有限梯度检查全部通过。
- 指标：分类固定样本周期均值 loss `0.124485 -> 0.059652`，检测为 `7.938776 -> 4.463577`；AMP scale 保持 32；峰值显存 `2,494,724,096` bytes（约 2.32 GiB）；CPU/CUDA 联合推理、固定预测严格重载和循环边界续跑均通过。
- 交接边界：J2 是链路和小样本可行性验收，不代表正式检测精度或分类抗遗忘达标。J3 必须读取固定 train/val，完整交替训练并分别报告分类 val 与检测 val；先满足分类 Top-1 相对 J1 下降不超过 2 个百分点，再按检测 mAP@0.5:0.95 选优。
...
- 结论：通过；此前尚未提交、尚未验收的单任务检测 T1 代码已回退，T0 数据合同保持不变。
- 用户决策：最终采用一个模型、一个 RGB 224 输入、一次共享 ResNet-50 前向和一个 checkpoint；先分类适配，再用 DLCPD-25 与 IP102 按 1:1 step 交替联合训练。
- 阶段判断：历史 203 类分类预训练已经完成，但其预处理为 resize 256 + center crop 224；统一直缩 224 的分类适配尚未完成，因此新增 J1，不能直接跳到联合训练。
- 新阶段：算法 J1-J4、应用 J5、总负责人 F1；数据工程师仅维护已经通过的 T0 合同。
- 目录与文档：统一重写项目计划、开发指南、模型设计、职责、流程、验收清单、工作单和启动提示词；未移动原始数据或冻结 artifacts。
- 下一阶段：用户授权算法工程师执行 J1。

## 2026-08-12 T0 验收

- 结论：通过；未执行 T1。
- 验收输入：数据工程师 T0 待验收交付，基线 commit `a0791e6`，产物 `artifacts/data/ip102-detection-v1/`。
- 独立复验：T0 `--verify-only` 退出 0，20 项 checksum 全部匹配；Dataset verifier 退出 0，完整遍历 18,976 张、22,283 个有效框，train/val DataLoader 冒烟通过；定向检测回归 `8 passed`；`git diff --check` 通过。
- 核心合同：train/val/test `12,142/3,036/3,798`，原始框 `22,284`，过滤 1 个零宽框后有效框 `22,283`；97 个源标签映射为 96 个检测类；官方 test 缺源类 61；5 张额外 JPEG 已记录并排除。
- 结论边界：原始图片/XML、官方 split、taxonomy、映射和分类模型包未修改；T0 产物可交接算法工程师。T1 可由用户授权启动。
